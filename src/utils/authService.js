...
class AuthService {
  ...
  async signOut() {
    try {
      ...
      localStorage.removeItem('smartHMS_session');
  ...
  async getHotelConfig() {
    try {
      ...
      return { 
        success: false, 
        error: 'Failed to load hotel configuration' 
      };
    }
  }

  // Update hotel configuration (admin only)
  async updateHotelConfig(updates) {
    try {
      const { data, error } = await supabase
        .from('hotel_config')
        .update(updates)
      ...
      return { 
        success: false, 
        error: 'Failed to update hotel configuration' 
      };
    }
  }
}
